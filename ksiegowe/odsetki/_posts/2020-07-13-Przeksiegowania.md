...
- Pole `Sposób` > wybrać: (pole aktywne tylko dla pola `Typ` > `Wg zakresu dat`)
  ...
  - `Przeksięgowanie bieżącego salda` > system nie analizuje kartoteki właściciela, robione jest przeksięgowanie salda na dzień poprzedzający dzień ustawiony w polu `Do dnia`.
- Pola `Od dnia (>=)` oraz `Do dnia (<=)` > zależnie od wcześniejszych ustawień, definiujemy okres lub dzień przeksięgowania. 
- Klikamy: `Zatwierdź`.
